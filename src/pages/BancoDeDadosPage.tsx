...
import { Database, Music, BookOpen, Search, Clock, User, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="relative max-w-md">
        <input
          ...
          placeholder={
            activeTab === 'songs'
              ? 'Pesquisar hino ou cantor no banco de dados...'
              : 'Pesquisar livro ou versículo no banco de dados...'
          }
                ...
                {loading ? (
                  ...
                  filteredSongs.map((song) => (
                    <tr key={song.id} className="hover:bg-slate-800/50 transition-colors group">
                      <td className="py-4 px-6 font-mono text-xs text-slate-500">{song.id}</td>
                      ...
                      <td className="py-4 px-6 font-medium text-slate-200 group-hover:text-[#d4af37] transition-colors">{song.songName}</td>
                ...
                {loading ? (
                  ...
                  filteredWords.map((word) => (
                    <tr key={word.id} className="hover:bg-slate-800/50 transition-colors group">
                      <td className="py-4 px-6 font-mono text-xs text-slate-500">{word.id}</td>
                      <td className="py-4 px-6 font-bold text-[#d4af37] flex items-center gap-2">
                        <BookOpen className="w-4 h-4 text-[#d4af37] shrink-0" />
                        <span>{word.book}</span>
                      </td>
                      <td className="py-4 px-6 font-semibold text-slate-100">Cap. {word.chapter}</td>
                      <td className="py-4 px-6 font-semibold text-slate-200">Vers. {word.verse}</td>
                      <td className="py-4 px-6 font-mono text-xs text-slate-400">
